...
import { FileText, Image, Headphones, Vote, Search, MoreHorizontal, Pencil, Trash2 } from "lucide-react";
...
const PostRow = ({ post, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-patron-white rounded-2xl border border-patron-gray-200 p-5">
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm text-patron-gray-400">{postTimestampLabel(post)}</p>
        <div className="relative" ref={ref}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full mt-1 w-36 bg-patron-white border border-patron-gray-200 rounded-xl shadow-xl py-1 z-10">
              <button
                onClick={() => {
                  setMenuOpen(false);
                  onEdit(post);
      ...
      <h3 className="font-bold text-patron-black mt-1">{post.title || "Untitled"}</h3>
